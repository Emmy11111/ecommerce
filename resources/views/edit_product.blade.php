...
<body>
    ...
    @include("../navbar")
    <?php $pr = $product[0]; ?>
    <form action="{{ route('edit.product', ['id'=>$pr->id]) }}" method="POST" class="row g-3 mx-auto col-xl-8 mb-5 p-4"
        enctype="multipart/form-data">
        @csrf
        <h4 class="fw-bold mt-4 mb-3 title-add">Edit product</h4>
        ...
        <div class="col-md-4">
            ...
            <select id="inputState2" name="category" class="form-select">
                ...
                @foreach ($product_type as $type)
                    <option @if($type->id == $product[0]->category) selected @endif value="{{ $type->id }}">{{ $type->type }}</option>
                @endforeach
            </select>
        ...
        <div class="col-md-4">
            ...
            <input type="text" name="title" id="inputEmail4" class="form-control" value="{{ $pr->title }}">
            <small class="error">@error('title'){{ $message }} @enderror</small>
        </div>
        <div class="col-md-4">
            <label for="inputPassword4" class="form-label">Price (RWF)</label>
            <input type="number" class="form-control" name="price" value={{ $product[0]->price }} id="inputPassword4">
            ...
            <div class="col-4">
                ...
                <select id="inputState" onchange="getSector(this)" name="district" class="form-select">
                    ...
                    @foreach ($districts as $district)
                        <option @if($product[0]->district == $district->districtcode) selected @endif value={{ $district->districtcode }}>{{ $district->namedistrict }}</option>
                    @endforeach
                </select>
            ...
            <div class="col-4">
                ...
                <select id="inputState5" onchange="getcell(this)" name="sector" class="form-select sectors">
                    ...
                    @foreach ($sectors as $sector)
                        <option @if($product[0]->sector == $sector->sectorcode) selected @endif value={{ $sector->sectorcode }} style="display: none;"
            ...
            <div class="col-4">
                ...
                <select id="inputState4" name="cell" class="form-select cells">
                    ...
                    @foreach ($cells as $cell)
                        <option @if($product[0]->cell == $cell->codecell) selected @endif value={{ $cell->codecell }} style="display: none;" class="{{ $cell->sectorcode }} cells-options">{{ $cell->nameCell }}</option>
                    @endforeach
                </select>
        ...
        <div class="col-12">
            ...
            <div class="add">
                ...
                <input class="form-check-input ms-3 me-1" @if($product[0]->phone == $userInfo->phone) checked @endif type="checkbox" id="gridCheck" name="phone-o"
        ...
        <div class="col-12">
            ...
            @if($product[0]->phone != $userInfo->phone) value={{ $product[0]->phone }} @endif>
            <small class="error">@error('phone'){{ $message }} @enderror</small>
        </div>

        <div class="col-12">
            <label for="formFile" class="form-label">Add photo</label>
            <input class="form-control" name="file" type="file" id="formFile" value="{{ $pr->file }}" onchange="priviewImage(event)">
            <small class="error">@error('file'){{ $message }} @enderror</small>
        </div>

        <input type="text" name="prevfile" style="display:none;" value="{{ $pr->file}}">

        <div class="col-12 image my-0" id="p-image">
            <img src="{{ $product[0]->file }}" id="image" class="w-100 my-4">
        </div>

        <div class="col-12 text-center">
            <button type="submit" class="btn w-50 add-btn ">Edit Product</button>
        </div>
    </form>
